...
export default function AdminTarget() {
    ...
    function getCourse() {
        axios.get(`${process.env.NEXT_PUBLIC_SERVER_API}/course`, { headers: { 'secret': process.env.NEXT_PUBLIC_SECRET, 'type': 'target' } }).then(({ data }) => {
            setTarget(data)
        }).catch(err => {
            console.log(err)
        ...
        <>
            ...
            <AdminLayout name='course'>
                ...
                <Rodal visible={rodal} onClose={rodalClose}>
                    <h3>target bo'limiga darslik qo'shish</h3>
